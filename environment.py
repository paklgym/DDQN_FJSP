...
class Environment:
    def __init__(self, file):
        ...
        for lines in content:
            ...
            content_formatted.append(lines)
            
        n_jobs = int(content_formatted[0].split(' ')[0])
        n_machines = int(content_formatted[0].split(' ')[1])

        ## Adiciona os valores das linhas do dataset em listas de jobs
        jobs_arr_np = [np.array([int(x) for x in line.split()]) for line in content_formatted[1:] if len(line) > 0]

        ## Monta a configuração das máquinas do dataset
        no_machines = int(content_formatted[0].split(' ')[1])
    ...
    def start_jobs(self):
        ...
        # Se a máquina não está processando e possui job na fila
            if machine.processing == False and len(machine.queue) > 0:
                ...
                #print(str(self.time) + ' Iniciando job na Máquina ' + str(machine.id))
                ...
                for machine_op in operation.machine_ops:
                    if machine_op.machine_no == machine.id:
                        #print('Job '+ str(job.id) + ' Operacao ' + str(operation.id) + ' Tempo de processamento ' + str(machine_op.proc_time) + ' máquina ' + str(machine.id))
